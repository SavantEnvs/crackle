...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    /* Scratch the fuzz bytes to a temp pcap so we can drive crackle's file-based
     * parser in-process. Mayhem mounts the whole image READ-ONLY during coverage
     * collection — /tmp, the CWD, /mayhem are all read-only; the ONLY writable
     * location is /dev/shm (a kernel tmpfs Docker mounts by default). Writing to
     * /tmp fails with EROFS, so mkstemp returns -1 and every input bails before
     * the parser runs -> 0 edges. Stage the temp file under /dev/shm instead. */
    char path[] = "/dev/shm/crackle_fuzz_XXXXXX";
    int fd = mkstemp(path);
    if (fd < 0)
        return 0;
